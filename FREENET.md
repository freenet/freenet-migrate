...
**The pre-publish gate has been run and passed (2026-08-17), so the first publish is unblocked.** The artifact is frozen and CI-enforced, and this contract's logic has now been executed on `wasm32` against a real node: 10 checks, 0 failures, covering `validate_state`'s accept and reject paths, a stale record, a forged record, and a control proving the forgery was refused for its signature rather than for its version. One caveat is recorded honestly with the run: `update_state` was exercised in local mode only, because a single network-mode node currently fails every UPDATE with `missing contract` ([freenet-core#5361](https://github.com/freenet/freenet-core/issues/5361)), which is a node bug and not a fault in this contract. The evidence is in the corresponding box in [`contracts/pointer-contract/README.md`](contracts/pointer-contract/README.md).
