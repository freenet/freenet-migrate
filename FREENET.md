...
**LIVE since 2026-08-18.** River publishes the first two records — `river.room-contract` at `Ai4VLoC2jGdhpcB2UU8VPo3efUoxjm1Ju9VKXqRC63Az` and `river.chat-delegate` at `6qF2H5JRPBxbKC45UtPnzdDzyfsejYFW1UwDLGDU66mu` — both verified from the network and resolved through `resolve_app_pointer` after publishing. **The pre-publish gate was run and passed (2026-08-17).** The artifact is frozen and CI-enforced, and this contract's logic has now been executed on `wasm32` against a real node: 10 checks, 0 failures, covering `validate_state`'s accept and reject paths, a stale record, a forged record, and a control proving the forgery was refused for its signature rather than for its version. One caveat is recorded honestly with the run: `update_state` was exercised in local mode only, because a single network-mode node currently fails every UPDATE with `missing contract` ([freenet-core#4978](https://github.com/freenet/freenet-core/issues/4978)), which is a node bug and not a fault in this contract. The evidence is in the corresponding box in [`contracts/pointer-contract/README.md`](contracts/pointer-contract/README.md).
